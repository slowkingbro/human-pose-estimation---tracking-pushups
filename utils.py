...
def get_frame_pic(timestamp, name_root, is_good_pushup, fps = 25):
  ...
  kp_2d = get_pkl_item(vibe_output, frame_number, item='joints2d_img_coord')
...
def is_straight_back(vibe_output, frame_number, if_print = False): #check both hip and the right knee on the same plane as right/left shoulder and left knee
  ...
  # X, Y, Z = rknee
  # Z_pred = (d - a * X - b * Y) / c
  # rknee_error = abs(Z_pred - Z)
  # return max(lhip_error, lhip_error)
  # if print:
  #   print('rshoulder:', rshoulder, '/n')
  #   print('lshoulder:', lshoulder, '/n')
  #   print('lknee:', lknee, '/n')
    
  #   print('lknee - rshoulder:', v1, '/n')
  #   print('lshoulder - rshoulder:', v2, '/n')
...
def identify_person_key(vibe_output, frame_number):  
  people_key = 0
  for i in vibe_output.keys():
    #print('vibe keys all:', i)
    if frame_number in vibe_output[i]['frame_ids']:
      #print('frame', frame_number, 'in key = ', i)
      people_key = i
      break
  #print('identify_function return:', people_key)
  return people_key


def get_pkl_item (vibe_output, frame_number, item = 'joints3d'):
# joins reference github https://github.com/mkocabas/VIBE/blob/6e70f5fcba8809fb64e4199bbfd44fb324ac47e4/lib/data_utils/kp_utils.py
  people_key = identify_person_key(vibe_output, frame_number)
  #print('key_from_get_pkl_item', people_key)
  if people_key != 0:
...
def count_pushup(name_root, timestamp_start, timestamp_end, is_good_pushup = 'BAD', tracking_joint = 2, save_optical_flow = False, fps = 25): # by default tracking joint = 2 to track rshoulder
  ...
  kp_2d = get_pkl_item(vibe_output, frame_range[0], item='joints2d_img_coord')
...
def is_error_spike(vibe_output, timestamp):
  ...
  print(mean, std, df_error['error'].max())
  print((df_error['error'].max()>3*std+mean))
  return (df_error['error'].max()>3*std+mean), min
  ...
  for i in range((timestamp+1)*25, (timestamp+2)*25):
    t1 = i
    points_im1 = get_pkl_item(vibe_output, t1, item = 'joints2d_img_coord')
    t2 = i+5
    points_im2 = get_pkl_item(vibe_output, t2, item = 'joints2d_img_coord')
...
structure_test(vibe_output, 107)
